...
import { IconDashboard, IconAttendance, IconFees, IconSignOut, IconClose } from '@/components/ui/Icons'

const NAV = [
  { to: '/student/dashboard',  label: 'Dashboard',  Icon: IconDashboard },
  { to: '/student/attendance', label: 'Attendance', Icon: IconAttendance },
  ...
  const SidebarContent = () => (
    <>
      <div className="h-16 flex items-center px-5 border-b border-white/10 shrink-0">
        ...
        <span className="ml-auto text-[10px] font-medium text-violet-400 bg-violet-500/10 px-2 py-0.5 rounded-full">Student</span>
      ...
      <div className="p-3 border-t border-white/10 shrink-0">
        <div className="flex items-center gap-2.5 p-2 rounded-lg">
          <div className="w-8 h-8 bg-violet-600 rounded-full flex items-center justify-center shrink-0">
            <span className="text-white text-xs font-semibold">{user?.full_name?.[0] ?? 'S'}</span>
          </div>
